...
func (chain *ChainBridge) FLendGetLendingPool() (*contracts.ILendingPool, error) {
	// get the lending pool contract
	lp, err := contracts.NewILendingPool(chain.fLendCfg.lendigPoolAddress, chain.eth)
	if err != nil {
		chain.log.Errorf("Can not get lending pool contract on address %s; %s", chain.fLendCfg.lendigPoolAddress.String(), err.Error())
		return nil, err
	}
	return lp, nil
}
...
func (chain *ChainBridge) FLendGetLendingPoolReserveData(assetAddress *common.Address) (*types.ReserveData, error) {

	// get the lending pool contract
	lp, err := chain.FLendGetLendingPool()
	if err != nil {
		chain.log.Errorf("Can not access lending pool %s", err.Error())
		return nil, err
	}

	rd, err := lp.GetReserveData(&bind.CallOpts{}, *assetAddress)
	if err != nil {
		chain.log.Errorf("Cannot get reserve data for asset %s: %s", assetAddress.String(), err.Error())
		return nil, err
	}
...
func (chain *ChainBridge) FLendGetReserveList() ([]common.Address, error) {
	...
	if err != nil {
		chain.log.Errorf("Cannot get reserves list: %s", err.Error())
		return nil, err
	}
	return rl, nil
}
...
func (chain *ChainBridge) FLendGetUserAccountData(userAddress *common.Address) (*types.FLendUserAccountData, error) {
	...
	if err != nil {
		chain.log.Errorf("Cannot get user account data for address %s: %s", userAddress.String(), err.Error())
		return nil, err
	}

	uc, err := lp.GetUserConfiguration(&bind.CallOpts{}, *userAddress)
	if err != nil {
		chain.log.Errorf("Cannot get user account configuration data for address %s: %s", userAddress.String(), err.Error())
		return nil, err
	}
...
func (chain *ChainBridge) FLendGetUserDepositHistory(userAddress *common.Address, assetAddress *common.Address) ([]*types.FLendDeposit, error) {
	// create user filter
	...
	if err != nil {
		chain.log.Errorf("can not filter lending pool deposit logs: %s", err.Error())
		return nil, err
	}
	...
	for fdi.Next() {
		...
		blk, err := chain.BlockByHash(&blkHash)
		if err != nil {
			chain.log.Errorf("fLend block with hash %s was not found: %s", blkHash, err.Error())
			continue
		}
