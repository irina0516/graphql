...
func (chain *ChainBridge) GovernanceProposalsCount(gov *common.Address) (hexutil.Big, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, chain.eth)
	if err != nil {
		chain.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		chain.log.Errorf("can not count governance %s proposals; %s", gov.String(), err.Error())
		return hexutil.Big{}, err
	}
...
// specified by its id.
func (chain *ChainBridge) GovernanceProposal(gov *common.Address, id *hexutil.Big) (*types.GovernanceProposal, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, chain.eth)
	if err != nil {
		chain.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	// get the detail
	gp, err := chain.governanceProposalDetail(gc, gov, id.ToInt())
	if err != nil {
		chain.log.Errorf("proposal %d not available in %s; %s", id.ToInt().Uint64(), gov.String(), err.Error())
		return nil, err
	}
...
// GovernanceProposal provides a detail of Proposal of a governance contract
// specified by its id.
func (chain *ChainBridge) governanceProposalDetail(gc *contracts.Governance, govId *common.Address, id *big.Int) (*types.GovernanceProposal, error) {
	...
	ext, err := chain.GovernanceProposalDetails(&data.ProposalContract)
	if err != nil {
		return nil, err
	}
...
// specified by its id.
func (chain *ChainBridge) GovernanceProposalState(gov *common.Address, id *hexutil.Big) (*types.GovernanceProposalState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, chain.eth)
	if err != nil {
		chain.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		chain.log.Errorf("can not access governance %s proposal %d state; %s", gov.String(), id.ToInt().Int64(), err.Error())
		return nil, err
	}
...
// specified by its id.
func (chain *ChainBridge) GovernanceProposalDetails(prop *common.Address) (*govProposalExtended, error) {
	// get the proposal contract
	pp, err := contracts.NewGovernanceProposal(*prop, chain.eth)
	if err != nil {
		chain.log.Errorf("can not access governance proposal %s; %s", prop.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		chain.log.Errorf("governance proposal %s name not available; %s", prop.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		chain.log.Errorf("governance proposal %s description not available; %s", prop.String(), err.Error())
		return nil, err
	}
...
func (chain *ChainBridge) GovernanceOptionState(gov *common.Address, propId *hexutil.Big, optId *hexutil.Big) (*types.GovernanceOptionState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, chain.eth)
	if err != nil {
		chain.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	gs, err := chain.GovernanceOptionStateById(gc, propId, optId)
	if err != nil {
		chain.log.Errorf("governance %s proposal #%d state #%d not available; %s",
...
func (chain *ChainBridge) GovernanceOptionStates(gov *common.Address, propId *hexutil.Big, optRange int) ([]*types.GovernanceOptionState, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, chain.eth)
	if err != nil {
		chain.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	for i := 0; i < optRange; i++ {
		...
		res[i], err = chain.GovernanceOptionStateById(gc, propId, (*hexutil.Big)(big.NewInt(int64(i))))
		if err != nil {
			chain.log.Errorf("unknown option #%d on governance %s; %s", i, gov.String(), err.Error())
			break
		}
	}
	return res, nil
...
func (chain *ChainBridge) GovernanceOptionStateById(gc *contracts.Governance, propId *hexutil.Big, optId *hexutil.Big) (*types.GovernanceOptionState, error) {
	// get the state
...
func (chain *ChainBridge) GovernanceVote(
	gov *common.Address,
	propId *hexutil.Big,
	from *common.Address,
	...
	if err != nil {
		chain.log.Errorf("can not access vote of %s on governance %s; %s", from.String(), gov.String(), err.Error())
		return nil, err
	}
...
func (chain *ChainBridge) GovernanceProposalsBy(gov *common.Address) ([]*types.GovernanceProposal, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, chain.eth)
	if err != nil {
		chain.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	// log what we do
	chain.log.Noticef("loading %d proposals of %s", maxProposalId.Uint64(), gov.String())
	...
	for i := maxProposalId.Int64(); i > 0; i-- {
		...
		gp, err := chain.governanceProposalDetail(gc, gov, big.NewInt(i))
		if err != nil {
			chain.log.Errorf("can not access governance %s proposal %d; %s", gov.String(), i, err.Error())
			return nil, err
		}

		// keep the proposal in the list
		chain.log.Noticef("found proposal #%d on %s", gp.Id.ToInt().Uint64(), gov.String())
		result = append(result, gp)
	}

	// log what we do
	chain.log.Debugf("%d proposals of %s loaded", maxProposalId.Uint64(), gov.String())
	return result, nil
}

// GovernanceProposalFee returns the fee payable for a new proposal
// in given Governance contract context.
func (chain *ChainBridge) GovernanceProposalFee(gov *common.Address) (hexutil.Big, error) {
	// get the contract
	gc, err := contracts.NewGovernance(*gov, chain.eth)
	if err != nil {
		chain.log.Errorf("can not access governance %s; %s", gov.String(), err.Error())
	...
	if err != nil {
		chain.log.Errorf("governance %s fee not available; %s", gov.String(), err.Error())
		return hexutil.Big{}, err
	}
...
func (chain *ChainBridge) GovernanceTotalWeight(ge *common.Address) (*hexutil.Big, error) {
	// get the contract
	goe, err := contracts.NewGovernable(*ge, chain.eth)
	if err != nil {
		chain.log.Errorf("can not access governable adapter %s; %s", ge.String(), err.Error())
		return nil, err
	}

	// get the total available voting weight
	w, err := goe.GetTotalWeight(chain.DefaultCallOpts())
	if err != nil {
		chain.log.Errorf("total weight not available on adapter %s; %s", ge.String(), err.Error())
		return nil, err
	}
