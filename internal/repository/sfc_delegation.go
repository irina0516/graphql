...
import (
	"galaxy-graphql/internal/repository/db"
...
func (p *proxy) DelegationOutstandingSCoin(addr *common.Address, toStaker *hexutil.Big) (*hexutil.Big, error) {
	val, err := p.rpc.DelegationOutstandingSCoin(addr, toStaker.ToInt())
	if err != nil {
		return nil, err
	}
	return (*hexutil.Big)(val), nil
