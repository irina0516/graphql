...
type Repository interface {
	// Account returns account at blockchain for an address, nil if not found.
	Account(*common.Address) (*types.Account, error)

	// AccountBalance returns the current balance of an account at blockchain.
	AccountBalance(*common.Address) (*hexutil.Big, error)

	// AccountNonce returns the current number of sent transactions of an account at blockchain.
	AccountNonce(*common.Address) (*hexutil.Uint64, error)

	// AccountTransactions returns list of transaction hashes for account at blockchain.
	//
	...
	// BlockHeight returns the current height of the blockchain in blocks.
	...
	// BlockByNumber returns a block at blockchain represented by a number.
	...
	// BlockByHash returns a block at blockchain represented by a hash.
	...
	// Contracts returns list of smart contracts at blockchain.
	...
	// LoadTransaction returns a transaction at blockchain
	// by a hash loaded directly from the node.
	LoadTransaction(hash *common.Hash) (*types.Transaction, error)

	// Transaction returns a transaction at blockchain by a hash, nil if not found.
	Transaction(*common.Hash) (*types.Transaction, error)

	// Transactions returns list of transaction hashes at blockchain.
	...
	// LastValidatorId returns the last validator id in blockchain.
	LastValidatorId() (uint64, error)

	// ValidatorsCount returns the number of stakers in blockchain.
	...
	// DelegationOutstandingSCoin returns the amount of sCoin tokens for the delegation
	// identified by the delegator address and the staker id.
	DelegationOutstandingSCoin(*common.Address, *hexutil.Big) (*hexutil.Big, error)
