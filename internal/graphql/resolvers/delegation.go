...
// OutstandingSCoin resolves the amount of outstanding sCoin tokens
// minted for this account.
func (del Delegation) OutstandingSCoin() (hexutil.Big, error) {
	val, err := repository.R().DelegationOutstandingSCoin(&del.Address, del.Delegation.ToStakerId)
	if err != nil {
		return hexutil.Big{}, err
	}
	return *val, nil
